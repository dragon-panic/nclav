...
impl TerraformBackend {
    ...
    pub async fn provision(
        ...
        if let Some(source) = &tf_config.source {
            ...
            self.write_backend_tf(&workspace)?;
            write_tfvars(&workspace, resolved_inputs)?;
        }
...
/// Write `nclav_context.auto.tfvars` containing only the resolved partition inputs.
/// Nothing is injected automatically — every key present here was explicitly put in
/// `inputs:` (including any `{{ nclav_* }}` tokens the user opted in to).
fn write_tfvars(workspace: &Path, resolved_inputs: &HashMap<String, String>) -> Result<(), DriverError> {
